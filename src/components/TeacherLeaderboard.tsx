  ...
  return (
    <div className="mt-8">
      <div className="rounded-[1.75rem] border border-white/10 bg-slate-900/70 p-4 shadow-[0_16px_60px_rgba(15,23,42,0.25)] backdrop-blur">
          ...
          <div className="min-w-0 lg:mr-auto">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <h2 className="text-xl font-bold text-white sm:text-2xl">{m.teacherLeaderboard.controlsTitle}</h2>
              </div>
            </div>
          </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(0,1.3fr),minmax(190px,0.8fr),minmax(190px,0.8fr),auto,auto]">
          <label className="relative min-w-0 md:col-span-2 xl:col-span-1">
            ...
            <input
              ...
              className="w-full rounded-2xl border border-white/10 bg-slate-950/65 py-3 pl-11 pr-4 text-sm text-white outline-none transition focus:border-indigo-400/50"
            />
          </label>
          ...
          <select
            ...
            className="w-full min-w-0 rounded-2xl border border-white/10 bg-slate-950/65 px-4 py-3 text-sm text-white outline-none transition focus:border-indigo-400/50"
          >
          ...
          <button
            ...
            className={cn(
              'inline-flex w-full items-center justify-center gap-2 rounded-2xl border px-4 py-3 text-sm font-semibold transition-colors xl:w-auto',
              recentOnly
          ...
          <div
            className="flex items-center justify-stretch gap-1 rounded-2xl border border-white/10 bg-slate-950/45 p-1 xl:justify-self-end"
            role="group"
          ...
          >
            <button
              ...
              className={cn(
                'flex-1 rounded-xl px-3 py-2 text-sm font-semibold transition-colors xl:flex-none',
      ...
      {filteredTeachers.length === 0 ? (
        <div className="mt-8 rounded-[1.75rem] border border-dashed border-white/10 bg-slate-900/70 px-6 py-14 text-center shadow-[0_16px_60px_rgba(15,23,42,0.25)]">
        ...
        <>
          ...
          {view === 'list' ? (
            <div className="rounded-[1.75rem] border border-white/10 bg-slate-900/70 p-4 shadow-[0_16px_60px_rgba(15,23,42,0.25)] backdrop-blur">
              <div className="hidden md:grid md:grid-cols-[72px,minmax(0,1.3fr),minmax(0,0.8fr),120px,110px,120px] md:gap-3 border-b border-white/10 px-4 py-3 text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">
              ...
              <div className="mt-3 space-y-3">
                {filteredTeachers.map((teacher, index) => (
                  <motion.a
                    ...
                    className="grid grid-cols-1 gap-4 rounded-[1.5rem] border border-white/10 bg-slate-950/45 px-4 py-4 shadow-[0_10px_30px_rgba(15,23,42,0.25)] transition hover:border-indigo-400/30 focus-visible:border-indigo-300/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300/40 md:grid-cols-[72px,minmax(0,1.3fr),minmax(0,0.8fr),120px,110px,120px]"
                  >
                    <div className="flex items-center gap-3">
                      <span className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-indigo-500/20 text-sm font-black text-indigo-100">
                        #{index + 1}
                      </span>
                    </div>
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="break-words text-lg font-bold text-white sm:truncate">{teacher.full_name}</h3>
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {filteredTeachers.map((teacher, index) => (
                <motion.a
                  ...
                  onMouseEnter={(event) => setHover({ teacher, rect: event.currentTarget.getBoundingClientRect() })}
                  onMouseLeave={() => setHover(null)}
                  className="rounded-[1.75rem] border border-white/10 bg-slate-900/70 p-5 shadow-[0_16px_60px_rgba(15,23,42,0.25)] backdrop-blur transition hover:border-indigo-400/30 focus-visible:border-indigo-300/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300/40"
                >
