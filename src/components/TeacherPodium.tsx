...
function TeacherPodiumCard({
  ...
  const heights: Record<1 | 2 | 3, string> = {
    1: 'min-h-[18rem] sm:min-h-[19rem] md:min-h-[21rem]',
    2: 'min-h-[15.5rem] sm:min-h-[17rem] md:min-h-[19rem]',
    3: 'min-h-[15rem] sm:min-h-[16.5rem] md:min-h-[18rem]',
  };
  ...
  return (
    <motion.a
      ...
      className={cn(
        'relative flex w-full flex-col rounded-[1.75rem] border bg-gradient-to-br p-4 shadow-2xl transition-transform hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 sm:p-5',
        gradients[rank],
        heights[rank],
        layoutOrder[rank]
      )}
    ...
    >
      <div className="absolute -top-5 left-1/2 -translate-x-1/2 rounded-full bg-slate-950/90 p-2.5 shadow-lg ring-1 ring-white/10" aria-hidden="true">
        {rank === 1 ? (
      ...
      <div className="mt-4 flex h-18 w-18 items-center justify-center self-center rounded-[1.5rem] border border-white/20 bg-white/10 text-3xl font-black text-white/90 shadow-[inset_0_1px_0_rgba(255,255,255,0.12)] sm:h-20 sm:w-20">
        {teacher.full_name.charAt(0)}
      </div>

      <div className="mt-4 text-center">
        <h3 className="line-clamp-2 text-base font-black leading-tight text-white sm:text-lg">{teacher.full_name}</h3>
      ...
      <div className="mt-auto grid grid-cols-2 gap-2 pt-4 text-center">
        <div className="rounded-2xl border border-white/10 bg-black/10 px-2 py-2">
          <div className="text-[11px] uppercase tracking-[0.22em] text-white/55">{m.teacherLeaderboard.podiumStudents}</div>
          <div className="mt-1 text-lg font-black text-white">{teacher.unique_students_count}</div>
        </div>
        <div className="rounded-2xl border border-white/10 bg-black/10 px-2 py-2">
          <div className="text-[11px] uppercase tracking-[0.22em] text-white/55">{m.teacherLeaderboard.podiumDays}</div>
          <div className="mt-1 text-lg font-black text-white">{teacher.active_days_count}</div>
        </div>
      </div>
    </motion.a>
  );
