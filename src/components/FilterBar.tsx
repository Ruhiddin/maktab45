...
export default function FilterBar({
  ...
  return (
    <div
      className="sticky top-4 eboard:top-2 z-30 p-4 eboard:p-3 rounded-2xl bg-white/30 dark:bg-black/30 backdrop-blur-md border border-white/20 shadow-lg flex flex-wrap gap-3 eboard:gap-2 items-center"
      role="region"
      aria-label={m.public.filters}
    >
      <div className="flex items-center gap-3 eboard:gap-2 mr-auto flex-wrap">
        <h2 className="text-lg eboard:text-base font-bold text-gray-800 dark:text-gray-100">{m.public.filters}</h2>
        {showMyClass && (
          <button
            type="button"
            onClick={onMyClass}
            ...
            className={cn(
              'px-3 py-1 eboard:px-2.5 eboard:py-0.5 rounded-full text-xs font-semibold border transition-colors',
              myClassEnabled
        ...
        {yearOptions.length > 0 && (
          <div className="flex items-center gap-1 rounded-full border border-gray-200/80 bg-white/55 p-1 eboard:p-0.5 shadow-sm dark:border-white/10 dark:bg-black/25">
            {yearOptions.map((option) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'rounded-full px-3 py-1.5 eboard:px-2.5 eboard:py-1 text-xs font-semibold transition-colors whitespace-nowrap',
                    isActive
        ...
        <div className="flex items-center gap-2 eboard:gap-1.5 rounded-2xl border border-gray-200/80 bg-white/55 px-3 py-2 eboard:px-2.5 eboard:py-1.5 text-xs shadow-sm dark:border-white/10 dark:bg-black/25">
          ...
          <button
            ...
            disabled={refreshing}
            className={cn(
              'inline-flex items-center gap-1 rounded-full px-3 py-1.5 eboard:px-2.5 eboard:py-1 font-semibold transition-colors',
              refreshing
      ...
      <div className="flex flex-wrap gap-2 eboard:gap-1.5 w-full">
        <select
          aria-label={`${m.public.gradeSingle} ${m.public.sectionSingle}`}
          className="p-2 eboard:px-3 eboard:py-1.5 rounded-lg bg-white/50 dark:bg-black/50 border border-gray-200 dark:border-gray-700 outline-none flex-1 sm:flex-none text-sm eboard:text-[13px] text-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-indigo-400/60"
          value={classFilter}
        ...
        <input
          ...
          className="p-2 eboard:px-3 eboard:py-1.5 rounded-lg bg-white/50 dark:bg-black/50 border border-gray-200 dark:border-gray-700 outline-none flex-[2] eboard:flex-[1.4] text-sm eboard:text-[13px] min-w-[180px] eboard:min-w-[160px] text-gray-800 dark:text-gray-100 placeholder:text-gray-500 dark:placeholder:text-gray-400 focus:ring-2 focus:ring-indigo-400/60"
        />

        <div className="flex items-center gap-1 ml-auto bg-white/40 dark:bg-black/40 border border-gray-200 dark:border-gray-700 rounded-lg p-1 eboard:p-0.5">
          <button
            type="button"
            aria-pressed={view === 'list'}
            onClick={() => onToggleView('list')}
            className={cn(
              'px-3 py-1 eboard:px-2.5 eboard:text-[13px] rounded-md text-sm font-semibold transition-colors',
              view === 'list'
                ? 'bg-indigo-500 text-white shadow-sm'
